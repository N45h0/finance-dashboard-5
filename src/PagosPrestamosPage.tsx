...
import { Prestamo } from "./PrestamosPage";
...
export const PagosPrestamosPage: React.FC = () => {
  ...
  const { token } = useAuth();
  const [availableLoans, setAvailableLoans] = useState<Prestamo[]>([]);
  ...
  const fetchLoans = async () => {
    try {
      ...
      if (loansData.length > 0) {
        setForm(prev => ({...prev, loan_id: loansData[0].id.toString()}));
      }
    } catch(e) { console.error("Error al cargar los préstamos", e); }
  };
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 p-4 bg-white rounded-xl shadow-md max-w-lg mb-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <select name="loan_id" value={form.loan_id} onChange={handleChange} className="border rounded px-3 py-2 bg-white" required>
                  <option value="" disabled>-- Asociar a un préstamo --</option>
                  {availableLoans.map(loan => (
                    <option key={loan.id} value={loan.id}>{loan.loan_name}</option>
                  ))}
                </select>
      ...
      {!loading && !error && (
        <div className="w-full px-4 py-3">
          ...
          <div className="block lg:hidden space-y-4">
            {pagos.map((p) => {
              const loanName = availableLoans.find(loan => loan.id === p.loan_id)?.loan_name || 'Desconocido';
              return (
                <div key={p.id} className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
                  <div className="flex justify-between items-start">
                    <span className="font-bold text-lg text-[#121714]">{p.description || 'Pago de Préstamo'}</span>
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                      ...
                      <p><span className="text-gray-600">Para Préstamo:</span> {loanName}</p>
                  </div>
